...
def main():
    ...
    p.add_argument(
        "--split_mode", "-S", type=str, choices=["random", "subdirs"], default="random"
    )
    ...
    for i, (X_fname, y_fname) in enumerate(val_filelist):
        logger.info(
            "{} {} {}".format(
                i + 1, os.path.basename(X_fname), os.path.basename(y_fname)
            )
        )
    ...
    reduction_weight = (
        np.concatenate(
            [
                ...
                np.linspace(1, 0, stable_bins - unstable_bins, dtype=np.float32)[
                    :, None
                ],
